...
import {FilePenLineIcon, PlusIcon, TrashIcon, UploadCloudIcon, PencilIcon, XIcon, UploadCloud} from 'lucide-react'
  ...
  return (
    <div>
      <div className='max-w-7xl mx-auto px-4 py-8'>
        ...
        bg-clip-text text-transparent sm:hidden'>Welcome, Mansi Rathor</p>
        <div className='flex gap-4'> 
          ...
          duration-300 cursor-pointer'>
            ...
            duration-300'>Upload existiing</p>
          </button>
        </div>
        ...
        <div className='grid grid-cols-2 sm:flex flex-wrap gap-4'>
          {allResumes.map((resume, index)=>{
            ...
            return (
              <button key={index} onClick={()=> navigate(`builder/${resume._id}`)} className='relative w-full sm:max-w-36 h-48 flex
              flex-col items-center justify-center rounded-lg gap-2 border group hover:shadow-lg transition-all duration-300 cursor-pointer' style=
              ...
              `,borderColor: baseColor +'40'}}>
                ...
                <p className='absolute bottom-2 text-[11px] text-slate-400
                 group-hover:text-slate-500 transition-all duration-300 px-2
                  text-center' style={{color: baseColor +'90'}}>
                  Update on {new Date(resume.updateAt).toLocaleDateString()}
                </p>
                    ...
                    <input onChange={(e)=>setTitle(e.target.value)} value={title} type="text" placeholder='Enter resume title' className='w-full px-4 py-2 mb-4 focus:border-green-600 ring-green-600' required/>
                      ...
                      <div>
                        <label htmlFor="resume-input" className='block text-sm text-slate-700'>
                          ...
                          my-4 hover:border-green-500 hover:text-green-700 cursor-pointer transition-colors'>
                            {resume ? (
